...
if whiptail --backtitle "OSDL.SH" --title "Ubuntu Server 22.04 VM" --yesno "This will create a New Ubuntu Server 22.04 VM. Proceed?" 10 58; then
  :
else
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "OSDL.SH" --inputbox "Set Hostname" 8 58 ubuntu2204 --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="ubuntu2204"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else

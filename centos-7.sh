...
if whiptail --backtitle "OSDL.SH" --title "CentOS 7 VM" --yesno "This will create a New CentOS 7 VM. Proceed?" 10 58; then
  :
else
...
function default_settings() {
  ...
  echo -e "${BL}Creating an CentOS 7 VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "OSDL.SH" --inputbox "Set Hostname" 8 58 centos7 --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="centos7"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if (whiptail --backtitle "OSDL.SH" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create an CentOS 7 VM?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating an CentOS 7 VM using the above advanced settings${CL}"
  else
    header_info
...
msg_info "Retrieving the URL for the CentOS 7 Disk Image"
...
msg_info "Creating a CentOS 7 VM"
...
msg_ok "Created a CentOS 7 VM ${CL}${BL}(${HN})"

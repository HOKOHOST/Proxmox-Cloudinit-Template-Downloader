...
if whiptail --backtitle "OSDL.SH" --title "Alpine 3.21.0 VM" --yesno "This will create a New Alpine 3.21.0 VM. Proceed?" 10 58; then
  :
else
...
function default_settings() {
  ...
  echo -e "${BL}Creating an Alpine 3.21.0 VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "OSDL.SH" --inputbox "Set Hostname" 8 58 alpine3210 --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="alpine3210"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if (whiptail --backtitle "OSDL.SH" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create an Alpine 3.21.0 VM?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating an Alpine 3.21.0 VM using the above advanced settings${CL}"
  else
    header_info
...
msg_info "Retrieving the URL for the Alpine 3.21.0 Disk Image"
URL=https://dl-cdn.alpinelinux.org/alpine/v3.21/releases/cloud/generic_alpine-3.21.0-x86_64-uefi-cloudinit-r0.qcow2
sleep 2
msg_ok "${CL}${BL}${URL}${CL}"
...
msg_info "Creating a Alpine 3.21.0 VM"
...
msg_ok "Created a Alpine 3.21.0 VM ${CL}${BL}(${HN})"

...
if whiptail --backtitle "OSDL.SH" --title "Alpine 3.20.3 VM" --yesno "This will create a New Alpine 3.20.3 VM. Proceed?" 10 58; then
  :
else
...
function default_settings() {
  ...
  echo -e "${BL}Creating an Alpine 3.20.3 VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if (whiptail --backtitle "OSDL.SH" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create an Alpine 3.20.3 VM?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating an Alpine 3.20.3 VM using the above advanced settings${CL}"
  else
    header_info
...
msg_info "Retrieving the URL for the Alpine 3.20.3 Disk Image"
URL=https://dl-cdn.alpinelinux.org/alpine/v3.20/releases/cloud/generic_alpine-3.20.3-x86_64-uefi-cloudinit-r0.qcow2
sleep 2
msg_ok "${CL}${BL}${URL}${CL}"
...
msg_info "Creating a Alpine 3.20.3 VM"
...
msg_ok "Created a Alpine 3.20.3 VM ${CL}${BL}(${HN})"
